...
select count(emp_no) as '현재 근무 중인 직원 수' from salaries where to_date = '9999-01-01';

-- 문제5.
-- 부서는 총 몇 개가 있나요?
-- count(distinct)
select count(distinct dept_name) as '부서의 수' from departments;

-- 문제6.
-- 현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외 == 현재 매니저)
-- to_date이 9999-01-01가 아닌 매니저는 제외하라는 의미
...
select from_date, title from titles where emp_no=13250 order by from_date asc;
